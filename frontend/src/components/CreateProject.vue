    ...
    <v-row align="center" justify="space-between" class=" mb-2">
      ...
      <v-col cols="12" sm="6"
        v-if="user.id === user.id"><v-row
          ...
          align="center"
          class="ma-0"
          ><v-select label="Select project to invite" dense hide-details class="mr-5" outlined :items="admin.projects.map(proj => proj.name)"></v-select><v-btn color="primary" @click="$router.push('/user/edit')">Invite for projects <v-icon>mdi-pencil</v-icon></v-btn></v-row
      ></v-col>
    </v-row>
    <ViewProfile
    :user="user"
